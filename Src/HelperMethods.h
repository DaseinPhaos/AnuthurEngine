	...
	namespace Anuthur {
		...
		namespace D3D12Helper {
			...
			public:
				...
				static ResourceDescriptor Buffer(UINT64 sizeBytes,
					D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE,
					UINT64 alignment = 0);

				static ResourceDescriptor Tex1D(UINT64 width, DXGI_FORMAT format,
				...
				static ResourceDescriptor Tex2D(UINT64 width, UINT height,
					DXGI_FORMAT format, UINT16 arraySize = 1,
					D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE,
					UINT64 alignment = 0, UINT16 mipLevels = 1,
					D3D12_TEXTURE_LAYOUT layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR,
					UINT sampleCount = 1, UINT sampleQuality = 0);
				static ResourceDescriptor Tex3D(UINT64 width, UINT height,
