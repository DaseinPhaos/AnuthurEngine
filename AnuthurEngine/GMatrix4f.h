...
namespace Glyph3
{
	...
	public:
		...
		Matrix4x4f();
		Matrix4x4f( bool bZero );
		Matrix4x4f( const Matrix4x4f& Matrix );
		Matrix4x4f( float fM11, float fM12, float fM13, float fM14,
		...
		Matrix3x3f GetRotation() const;

		void SetRotation( const Matrix3x3f& Rot );
		void SetTranslation( const Vector3f& Trans );

		static Matrix4x4f RotationMatrixXYZ( float fRadiansX, float fRadiansY, float fRadiansZ );
		static Matrix4x4f RotationMatrixX( float fRadians );
		static Matrix4x4f RotationMatrixY( float fRadians );
		static Matrix4x4f RotationMatrixZ( float fRadians );
		static Matrix4x4f ScaleMatrix( float fScale );
		static Matrix4x4f ScaleMatrix( const Vector3f& scale );
		static Matrix4x4f ScaleMatrixXYZ( float fX, float fY, float fZ );
		static Matrix4x4f TranslationMatrix( float fX, float fY, float fZ );
		static Matrix4x4f LookAtLHMatrix( Vector3f& eye, Vector3f& at, Vector3f& up );
		static Matrix4x4f PerspectiveFovLHMatrix( float fovy, float aspect, float zn, float zf );
		static Matrix4x4f OrthographicLHMatrix( float zn, float zf, float width, float height );
		...
		Matrix4x4f operator- ( const Matrix4x4f& Matrix ) const;
		Matrix4x4f operator* ( const Matrix4x4f& Matrix ) const;
		Matrix4x4f operator* ( float fScalar ) const;
		Matrix4x4f operator/ ( float fScalar ) const;
		...
		Matrix4x4f& operator-= ( const Matrix4x4f& Matrix );
		Matrix4x4f& operator*= ( const Matrix4x4f& Matrix );
		Matrix4x4f& operator*= ( float fScalar );
		Matrix4x4f& operator/= ( float fScalar );
