...
//----------------------------------------------------------------------------------------------------
Matrix3x3f::Matrix3x3f( bool bZero )
{
	if ( bZero )
		memset( m_afEntry, 0, 3*3*sizeof(float) );
}
//----------------------------------------------------------------------------------------------------
Matrix3x3f::Matrix3x3f( const Matrix3x3f& Matrix )
{
	memcpy( m_afEntry, (void*)&Matrix, 9*sizeof(float) );
}
//----------------------------------------------------------------------------------------------------
Matrix3x3f::Matrix3x3f(	float fM11, float fM12, float fM13,
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::RotationX( float fRadians )
{
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::RotationY( float fRadians )
{
	float fSin = sinf( fRadians );
	float fCos = cosf( fRadians );
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::RotationZ( float fRadians )
{
	float fSin = sinf( fRadians );
	float fCos = cosf( fRadians );
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::Rotation( Vector3f& Rot )
{
	Matrix3x3f mRot1;
	Matrix3x3f mRot2;
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::RotationZYX( Vector3f& Rot )
{
	Matrix3x3f mRot1;
	Matrix3x3f mRot2;
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::RotationEuler( Vector3f& Axis, float Angle )
{
	float s = sinf( Angle );
	float c = cosf( Angle );
	float t = 1 - c;
...
//----------------------------------------------------------------------------
float Matrix3x3f::operator[] ( int iPos ) const
{
	return( m_afEntry[iPos] );
}
//----------------------------------------------------------------------------
float& Matrix3x3f::operator[] ( int iPos )
{
	return( m_afEntry[iPos] );
}
//----------------------------------------------------------------------------
float Matrix3x3f::operator() ( int iRow, int iCol ) const
{
	return( m_afEntry[I(iRow,iCol)] );
}
//----------------------------------------------------------------------------
float& Matrix3x3f::operator() ( int iRow, int iCol )
{
	return( m_afEntry[I(iRow,iCol)] );
}
//----------------------------------------------------------------------------
bool Matrix3x3f::operator== ( const Matrix3x3f& Matrix ) const
{
...
//----------------------------------------------------------------------------------------------------
Matrix3x3f& Matrix3x3f::operator*= ( float fScalar )
{
	for ( int i = 0; i < 3*3; i++ )
		m_afEntry[i] *= fScalar;
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::MakeZero()
{
	memset( m_afEntry,0,3*3*sizeof(float) );
}
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::MakeIdentity()
{
	for (int iRow = 0; iRow < 3; iRow++)
	{
		for (int iCol = 0; iCol < 3; iCol++)
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::MakeTranspose()
{
	Matrix3x3f mTranspose;
...
//----------------------------------------------------------------------------------------------------
Matrix3x3f Matrix3x3f::Zero()
{
	Matrix3x3f mReturn = Matrix3x3f( true );
...
//----------------------------------------------------------------------------------------------------
int Matrix3x3f::I( int iRow, int iCol )
{
	return( 3*iRow + iCol );
}
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::SetRow( int iRow, const Vector3f& Vector )
{
	for ( int iCol = 0; iCol < 3; iCol++ )
		m_afEntry[I(iRow,iCol)] = Vector[iCol];
}
//----------------------------------------------------------------------------------------------------
Vector3f Matrix3x3f::GetRow( int iRow ) const
{
	Vector3f vRow;
	for ( int iCol = 0; iCol < 3; iCol++ )
...
//----------------------------------------------------------------------------------------------------
void Matrix3x3f::SetColumn( int iCol, const Vector3f& Vector )
{
	for ( int iRow = 0; iRow < 3; iRow++ )
		m_afEntry[I(iRow,iCol)] = Vector[iRow];
}
//----------------------------------------------------------------------------------------------------
Vector3f Matrix3x3f::GetColumn( int iCol ) const
{
	Vector3f vCol;
	for ( int iRow = 0; iRow < 3; iRow++ )
...
float Glyph3::Matrix3x3f::Determinant() const
{
	const float* m = m_afEntry;
	return m[0] * (m[4] * m[8] - m[5] * m[7]) + m[1] * (m[5] * m[6] - m[3] * m[8]) + m[2] * (m[3] * m[7] - m[4] * m[6]);
}

Glyph3::Matrix3x3f Glyph3::Matrix3x3f::Inverse() const
{
